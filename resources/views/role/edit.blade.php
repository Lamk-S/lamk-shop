...
<div class="container-fluid px-4 py-4">
    <div class="mb-4">
        <h2 class="fw-bold text-dark mb-0">Modificar Rol</h2>
    ...
    <div class="card border-0 shadow-sm rounded-4 w-100 mx-auto" style="max-width: 900px;">
        <div class="card-header bg-white border-bottom border-light p-4 d-flex justify-content-between align-items-center">
            <h5 class="mb-0 fw-semibold text-dark"><i class="fa-solid fa-user-shield text-warning me-2"></i>Configuración del Rol y Permisos</h5>
            <span class="badge bg-light text-secondary border">ID: {{ $role->id }}</span>
        </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="name" id="name" class="form-control border-start-0 @error('name') is-invalid @enderror" value="{{ old('name', $role->name) }}">
                        </div>
                        @error('name')
                                ...
                                @foreach ($permisos as $item)
                                    <div class="col-md-6 col-lg-4">
                                        <div class="form-check form-switch p-3 border rounded-3 bg-white shadow-sm h-100 d-flex align-items-center">
                                            <input type="checkbox" name="permission[]" id="perm_{{ $item->id }}" class="form-check-input ms-0 me-3" style="width: 2.5em; height: 1.2em; cursor: pointer;" value="{{ $item->id }}" {{ in_array($item->id, $rolePermissions) ? 'checked' : '' }}>
                    ...
                    <div class="col-12 mt-5 d-flex justify-content-between align-items-center border-top pt-4">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button type="submit" class="btn btn-primary px-4 shadow-sm"><i class="fas fa-sync-alt me-2"></i>Actualizar Rol</button>
                        </div>
                    </div>
                </div>
            </form>
