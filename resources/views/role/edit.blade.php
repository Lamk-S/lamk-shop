...
<div class="container-fluid px-3 px-md-4 py-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <div>
            <h2 class="fw-bolder text-dark mb-0 fs-3">Configurar Perfil de Acceso</h2>
            <ol class="breadcrumb mb-0 mt-1 fs-7">
                ...
                <li class="breadcrumb-item active fw-medium text-dark">{{ $role->name }}</li>
            </ol>
        </div>
        <div class="d-flex align-items-center gap-2">
            <span class="badge bg-light text-secondary border px-3 py-2 fs-7 font-monospace shadow-sm">ID ROL: #{{ str_pad($role->id, 3, '0', STR_PAD_LEFT) }}</span>
    ...
    <div class="card border-0 shadow-sm rounded-4 mx-auto" style="max-width: 1200px;">
        <div class="card-header bg-white border-bottom p-4">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h5 class="mb-0 fw-bold text-dark">Modificación de Matriz de Accesos</h5>
                ...
                <div class="card border border-secondary border-opacity-25 rounded-4 p-4 mb-4 bg-white shadow-sm">
                    <div class="row align-items-end g-3">
                        <div class="col-lg-6">
                            ...
                            <input type="text" name="name" id="name" class="form-control form-control-lg fs-6 fw-bold text-dark border-secondary-subtle @error('name') is-invalid @enderror" value="{{ old('name', $role->name) }}" placeholder="Ej. Administrador de Ventas" {{ strtolower($role->name) === 'administrador' ? 'readonly' : '' }}>
                    ...
                    @if(strtolower($role->name) === 'administrador')
                        <div class="alert alert-warning bg-warning bg-opacity-10 border-warning border-opacity-25 text-dark small fw-medium mt-3 mb-0 p-2 rounded-3">
                            <i class="fas fa-shield-alt text-warning me-2"></i> El nombre del rol principal del sistema no puede ser modificado.
                        </div>
                    @endif
                </div>
                                    ...
                                    @foreach($permisos as $permiso)
                                        <div class="list-group-item d-flex justify-content-between align-items-center p-3 permission-item" data-perm-name="{{ strtolower($permiso->name) }}">
                                            ...
                                            <div class="form-check form-switch m-0 ms-2">
                                                <input class="form-check-input perm-checkbox" type="checkbox" role="switch" name="permission[]" value="{{ $permiso->id }}" id="perm-{{ $permiso->id }}" @checked(in_array($permiso->id, old('permission', $selectedPermissions ?? [])))>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
