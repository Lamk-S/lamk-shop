                ...
                <table id="datatablesSimple" class="table table-hover table-custom">
                    ...
                    <tbody>
                        @foreach ($compras as $item)
                            <tr>
                                <td>
                                    <div class="fw-medium text-dark">{{ $item->comprobante->tipo_comprobante }}</div>
                                    <div class="text-muted small">N° {{ $item->numero_comprobante }}</div>
                                </td>
                                <td>
                                    <div class="fw-medium text-dark">{{ $item->proveedore->persona->razon_social }}</div>
                                    <div class="text-muted small">{{ ucfirst($item->proveedore->persona->tipo_persona) }}</div>
                                </td>
                                <td>
                                    <div class="text-dark"><i class="far fa-calendar-alt me-1 text-muted"></i> {{ \Carbon\Carbon::parse($item->fecha_hora)->format('d/m/Y') }}</div>
                                    <div class="text-muted small"><i class="far fa-clock me-1"></i> {{ \Carbon\Carbon::parse($item->fecha_hora)->format('H:i') }} hrs</div>
                                </td>
                                ...
                                <td class="text-center align-content-center">
                                    <div class="btn-group shadow-sm" role="group">
                                        @can('mostrar-compra')
                                        <a href="{{ route('compras.show', ['compra' => $item]) }}" class="btn btn-sm btn-outline-secondary text-primary border-light" title="Ver detalles">
