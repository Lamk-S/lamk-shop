...
<div class="container-fluid px-4 py-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 no-print">
        <div>
            <h2 class="fw-bold text-dark mb-0">Detalles de la Compra</h2>
        ...
        <div class="mt-3 mt-md-0 d-flex gap-2">
            ...
            @if($compra->cuentaPorPagar && in_array($compra->cuentaPorPagar->estado, ['PENDIENTE', 'PARCIAL'], true))
                ...
                <div class="card-body p-4 p-md-5 bg-light bg-opacity-50">
                    <div class="row g-4">
                        ...
                        <div class="col-md-3 col-sm-6">
                            ...
                            <p class="summary-value">{{ $compra->serie && $compra->correlativo ? $compra->serie . '-' . $compra->correlativo : '—' }}</p>
                        </div>
                        <div class="col-md-3 col-sm-6">
                        ...
                        <div class="col-md-3 col-sm-6">
                            ...
                            <p class="summary-value">{{ $compra->proveedor_nombre ?? optional($compra->proveedor?->persona)->nombre_completo ?? 'Sin proveedor' }}</p>
                        </div>
                        <div class="col-md-3 col-sm-6">
                            <p class="summary-label">Documento proveedor</p>
                            <p class="summary-value">{{ $compra->proveedor_tipo_documento ?? 'N/A' }} {{ $compra->proveedor_numero_documento ?? '' }}</p>
                        </div>
                        <div class="col-md-3 col-sm-6">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body p-4">
                    <div class="mb-3">
                        ...
                        <span class="badge bg-{{ $compra->estado_pago === 'PAGADA' ? 'success' : ($compra->estado_pago === 'PARCIAL' ? 'warning' : ($compra->estado_pago === 'ANULADA' ? 'secondary' : 'danger')) }} bg-opacity-10 text-{{ $compra->estado_pago === 'PAGADA' ? 'success' : ($compra->estado_pago === 'PARCIAL' ? 'warning' : ($compra->estado_pago === 'ANULADA' ? 'secondary' : 'danger')) }} border px-3 py-2 rounded-pill">
                            {{ $compra->estado_pago }}
                        </span>
                    </div>
                    ...
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <strong>S/ {{ number_format((float) $compra->monto_pagado, 2) }}</strong>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
                        <span class="text-muted">Pendiente:</span>
                        <strong class="text-danger">S/ {{ number_format((float) $compra->saldo_pendiente, 2) }}</strong>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="mb-2">
                        ...
                        <div class="summary-value">{{ $compra->proveedor_tipo_documento ?? '—' }} {{ $compra->proveedor_numero_documento ?? '' }}</div>
                    </div>
                    <div class="mb-2">
            ...
            @if($compra->cuentaPorPagar)
                <div class="card border-0 shadow-sm rounded-4">
                    ...
                    <div class="card-body p-4">
                        ...
                        @if($compra->cuentaPorPagar->pagos->count())
                            <hr>
                            <div class="small fw-semibold text-muted mb-2">Pagos registrados</div>
                            <div class="vstack gap-2">
                                @foreach ($compra->cuentaPorPagar->pagos as $pago)
            ...
            <form method="POST" action="{{ route('cuentas-por-pagar.pagos.store', $compra->cuentaPorPagar) }}">
                ...
                <div class="modal-body">
                    <div class="alert alert-light border mb-3">
                        ...
                        <div class="fw-bold fs-5">S/ {{ number_format((float) $compra->cuentaPorPagar->saldo_pendiente, 2) }}</div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" min="0.01" max="{{ $compra->cuentaPorPagar->saldo_pendiente }}" name="monto" class="form-control" required>
                        @error('monto')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
