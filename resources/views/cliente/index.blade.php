...
<div class="container-fluid px-4 py-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div>
            <h2 class="page-title mb-0">Directorio de Clientes</h2>
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4 bg-white">
        <div class="card-body p-4 bg-light bg-opacity-50">
            <form method="GET" action="{{ route('clientes.index') }}" id="filtro-form" class="row g-3 filters-row">
                ...
                <div class="col-lg-2 col-md-4 d-flex align-items-end">
                    <a href="{{ route('clientes.index') }}" class="btn btn-outline-secondary w-100 fw-bold bg-white shadow-sm">
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-header bg-white border-bottom border-light p-4 d-flex align-items-center gap-3">
            ...
            <div>
                ...
                <div class="text-muted small">Personas naturales y empresas registradas para facturación y envíos.</div>
            </div>
        </div>
                    ...
                    <tbody>
                        @forelse ($clientes as $item)
                            ...
                            <tr>
                                ...
                                <td class="text-center py-3">
                                    @if($estaEliminado)
                                        <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 px-3 py-1 rounded-pill shadow-sm">Inactivo</span>
                                    @else
                                        <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-3 py-1 rounded-pill shadow-sm">Activo</span>
                                    @endif
                                </td>
                                @can('gestionar_clientes')
                                ...
                                <td colspan="{{ auth()->user()->can('gestionar_clientes') ? 6 : 5 }}" class="py-5">
                                    <div class="d-flex flex-column align-items-center justify-content-center text-center">
                                        <div class="bg-light rounded-circle d-flex align-items-center justify-content-center shadow-sm mb-3" style="width: 80px; height: 80px;">
                                            <i class="fas fa-user-tag text-muted fs-2 opacity-50"></i>
                                        </div>
            ...
            <div class="modal-content border-0 shadow-lg rounded-4">
                ...
                <div class="modal-body text-center p-4 pb-5">
                    @if($estaEliminado)
                        ...
                        <h4 class="fw-bold text-dark">¿Restaurar acceso comercial?</h4>
                        <p class="text-muted mb-4">El cliente <strong>{{ $nombreMostrado }}</strong> volverá a estar disponible para emitirle comprobantes.</p>
                    @else
                        <div class="text-danger mb-3"><i class="fas fa-ban fa-4x opacity-75"></i></div>
                        <h4 class="fw-bold text-dark">¿Bloquear cliente?</h4>
                        <p class="text-muted mb-4">El cliente <strong>{{ $nombreMostrado }}</strong> pasará a estar inactivo en la caja.</p>
                    @endif
