                ...
                <table id="datatablesSimple" class="table table-hover table-custom">
                    ...
                    <tbody>
                        @foreach($categorias as $categoria)
                            <tr>
                                <td class="fw-medium text-dark">{{ $categoria->caracteristica->nombre }}</td>
                                <td>{{ Str::limit($categoria->caracteristica->descripcion, 60, '...') }}</td>
                                ...
                                <td class="text-center align-content-center">
                                    <div class="btn-group shadow-sm" role="group">
                                        ...
                                        <a href="{{ route('categorias.edit', ['categoria' => $categoria]) }}" class="btn btn-sm btn-outline-secondary text-primary border-light" title="Editar">
                                    ...
                                    <div class="modal-content border-0 shadow">
                                        ...
                                        <div class="modal-body text-center pb-4">
                                            @if($categoria->caracteristica->estado == 1)
                                                ...
                                                <p class="text-muted">El registro "{{ $categoria->caracteristica->nombre }}" ya no estará disponible para nuevas transacciones.</p>
                                            @else
                                                <div class="text-success mb-3"><i class="fas fa-check-circle fa-4x"></i></div>
                                                <h4 class="fw-bold text-dark">¿Restaurar categoría?</h4>
                                                <p class="text-muted">El registro "{{ $categoria->caracteristica->nombre }}" volverá a estar activo en el sistema.</p>
                                            @endif
                                        </div>
                                        <div class="modal-footer border-0 pt-0 justify-content-center">
                                            ...
                                            <form action="{{ route('categorias.destroy', ['categoria' => $categoria->id]) }}" method="post">
                                                @method('DELETE')
                                                @csrf
                                                <button type="submit" class="btn {{ $categoria->caracteristica->estado == 1 ? 'btn-danger' : 'btn-success' }} px-4">
                                                    Confirmar
                                                </button>
                                            </form>
                                        </div>
