...
<div class="container-fluid px-4 py-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <div>
            <h2 class="page-title text-dark mb-0">Catálogo de Categorías</h2>
        ...
        @can('gestionar_categorias')
            <a href="{{ route('categorias.create') }}" class="btn btn-primary shadow-sm rounded-3 px-4 fw-medium">
        ...
        <div class="card-header soft-header p-4">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-muted small">Búsqueda rápida, filtros y mantenimiento de registros.</div>
                </div>
            </div>
        </div>

        <div class="card-body p-4">
            <form method="GET" action="{{ route('categorias.index') }}" class="row g-3 filters-row mb-4 bg-light rounded-3 p-3 border">
                ...
                <div class="col-lg-2 col-md-6 d-flex align-items-end gap-2">
                    <a href="{{ route('categorias.index') }}" class="btn btn-light border w-100 fw-medium">Limpiar</a>
                                ...
                                @can('gestionar_categorias')
                                    <td class="text-center">
                                        <div class="btn-group shadow-sm table-actions" role="group">
                                            <a href="{{ route('categorias.edit', $categoria) }}" class="btn btn-outline-secondary text-primary border-light" title="Editar">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <button type="button"
                                                    class="btn btn-outline-secondary {{ $categoria->trashed() ? 'text-success' : 'text-danger' }} border-light"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#confirmModal-{{ $categoria->id }}"
                                                    title="{{ $categoria->trashed() ? 'Restaurar' : 'Desactivar' }}">
                                ...
                                <td colspan="{{ auth()->user()->can('gestionar_categorias') ? 4 : 3 }}" class="py-5">
                                    <div class="empty-state d-flex flex-column align-items-center justify-content-center text-center">
                                        <div class="bg-light rounded-circle d-flex align-items-center justify-content-center shadow-sm mb-3" style="width: 90px; height: 90px;">
                                            <i class="fas fa-tags text-secondary fs-1 opacity-50"></i>
                                        </div>
                                        <h5 class="fw-semibold text-dark mb-1">No se encontraron categorías</h5>
                                        <p class="text-muted mb-0">Prueba con otros filtros o registra una nueva categoría.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mt-4 pt-3 border-top">
                <div class="text-muted small fw-medium">
                    Mostrando <span class="fw-bold text-dark">{{ $categorias->firstItem() ?? 0 }}</span> al <span class="fw-bold text-dark">{{ $categorias->lastItem() ?? 0 }}</span> de <span class="fw-bold text-dark">{{ $categorias->total() }}</span> registros
                </div>
                ...
                <div class="modal-content border-0 shadow">
                    ...
                    <div class="modal-body text-center pb-4">
                        @if(!$categoria->trashed())
                            ...
                            <p class="text-muted">La categoría <strong>{{ $categoria->nombre }}</strong> ya no estará disponible para nuevas transacciones.</p>
                        @else
                            <div class="text-success mb-3">
                                <i class="fas fa-check-circle fa-4x"></i>
                            </div>
                            <h4 class="fw-bold text-dark">¿Restaurar categoría?</h4>
                            <p class="text-muted">La categoría <strong>{{ $categoria->nombre }}</strong> volverá a estar activa en el sistema.</p>
                        @endif
                    </div>
